 ...
 */
class COMLoadingLayer extends AnimationLayer {
    /**
     * LoadingLayer类构造函数
    ...
    loop () {
        ...
        var _outter_color          = _loadingOptions.outter_color,
            _outter_line_width     = Util.getPxNumber(_loadingOptions.outter_line_width),
            _outter_radius         = Util.getPxNumber(_loadingOptions.outter_radius),
            _offsetX               = Util.getPxNumber(_loadingOptions.offsetX),
            _offsetY               = Util.getPxNumber(_loadingOptions.offsetY),
            _circles               = _loadingOptions.circles,
            _centerX               = _loadingOptions.centerX,
            _centerY               = _loadingOptions.centerY;
        var originX = (typeof _centerX === 'string' && _centerX === 'center') ? (_w / 2) : Util.getPxNumber(_centerX);
        var originY = (typeof _centerY === 'string' && _centerY === 'center') ? (_h / 2) : Util.getPxNumber(_centerY);
        // 第一步：清空画布
        self.clearLayer();
        // 第二步：绘制外围圆圈
        CanvasUtil.drawPoint(this.id, {x: originX + _offsetX, y: originY + _offsetY}, {
            type: '1',
        ...
        for (var i = 0; i < _circlesLength; i++) {
            ...
            CanvasUtil.drawPoint(this.id, {x: x + _offsetX, y: y + _offsetY}, {
                type: '2',
                fillStyle: '#fff',
